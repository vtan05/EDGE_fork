...
def set_on_ground(root_pos, local_q_156, smplx_model, floor_height=-1.2):
...
def set_on_ground_139(data, smplx_model, floor_height=-1.2):
    length = data.shape[0]
    ...
    if abs(l_toe_h - r_toe_h) < 0.02:
        ...
        height = min(l_toe_h, r_toe_h)
    data[:, 5] = data[:, 5] - height

    return data


def do_smplxfk(data, smplx_model):
    # data : N, 139 or B,T,139
    dimen = 0
    if not data.shape[-1] == 139:
    ...
    assert len(data.shape) == 2
    length ,_ = data.shape
    poses = data[:,7:139].reshape(length, 22, 6)
    # DEBUG
    # poses = poses[:,:,:3]
    poses = ax_from_6v(poses)
    hand = torch.zeros([length, 30, 3]).to(poses)
...
class SMPLX_Skeleton:
    def __init__(
        self, device=None, batch=64, Jpath = '/host_data/van/EDGE/data/finedance/smplx_neu_J_1.npy'
    ):
        self.parents = smplx_parents
        self.J = np.load(Jpath)
        self.J = torch.from_numpy(self.J).unsqueeze(dim=0).repeat(batch, 1, 1)
    ...
    def forward(self, rotations, root_positions):
        ...
        # self.J.to(fk_device)
        J_temp = self.J
        J_temp = J_temp.to(fk_device)
        # print("fk_device", fk_device)
        # print("J_temp device", J_temp.device)
        if J_temp.shape[0] > B:
            J_temp = J_temp[:B,:,:]
        else:
            J_temp = J_temp[:1,:,:].repeat(B,1,1)
